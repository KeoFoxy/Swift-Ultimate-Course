...
В этом примере мы определили класс `MyClass` с двумя свойствами: `property1` и `property2`, а также с методом `myMethod`. Класс также имеет инициализатор, который устанавливает значения для свойств.

Классы могут наследовать друг от друга с помощью ключевого слова `inheritance` (наследование). Это позволяет создавать более специализированные версии существующих классов или расширять функциональность существующих классов.  
...
Здесь мы создали подкласс `MySubclass`, который наследует свойства и методы класса `MyClass`. В подклассе мы переопределили метод `myMethod()`.

Классы в Swift также поддерживают протоколы, что позволяет определять общие требования для классов, структур и перечислений.
...
class MyProtocolClass: MyProtocol {
    func myProtocolMethod() {
        print("This is a method that conforms to MyProtocol")
    }
}
```  

Здесь мы определили протокол `MyProtocol`, который требует реализации метода `myProtocolMethod()`. Затем мы создали класс `MyProtocolClass`, который реализует этот метод.

Классы в Swift также поддерживают многопоточность, что позволяет выполнять асинхронные операции без блокировки главного потока выполнения.

Классы в Swift используются очень часто для создания объектов с различной функциональностью. Они предоставляют более высокий уровень абстракции, чем структуры, и могут быть более эффективными для работы с большими объемами данных.  

Об этом и многом другом будет рассказано подробнее далее.