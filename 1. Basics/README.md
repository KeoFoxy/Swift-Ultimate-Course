...
Когда нибудь вы сталкнетесь с ситуацией, что какая-то переменная или константа не будет иметь значения. Но как тогда обозначить отсутствие значения? Для этого существуют `optional`.  
...
Такая запись означает, что переменная хранит пустоту - `nil`. В дальнейшем эту переменную можно запомнить каким-либо значением.  

### Unwrapping optionals

К примеру, если у нас есть переменная:  

```swift
var name: String?
```

Что будет, если мы захотим вызвать `count`? У такой строки ничего нет, следовательно и считать нечего. А это не безопасно, swift не позволит этого сделать, но есть способ заглянуть внутрь переменной и узнать, что там находится. Такой процесс называют - ***unwrapping***.   

Самый распространенный способ - конструкция `if let`:  
...
Если `name` хранит в себе какое то значение, то оно попадет в `unwrapped` как обычная строка и затем выполнится `count`, но если же там nil, то отработает блок `else`.

### Unwrapping with guard  

Альтернатива конструкции `if let` является `guard let`. 

Главным отличием этих конструкций является то, что `guard let` распакованный опционал можно дальше использовать после конструкции `guard`.   

```swift
func connect(_ player: String?) -> String {
    guard let unwrapped = player else { 
        return "\(player) disconnected to the server" 
    }

    return "\(unwrapped) connected to the server"
}
```

В данном примере, если `guard` не встречает `nil`, то код функции исполняется дальше, но если иначе, то выходит из нее.   

### Force unwrapping  

Если вы точно уверены, что значение не `nil`, то Swift позволяет вам принудительно распаковать `optional`.  Делается это с помощью знака `!`.   
...
Но если там вдруг окажется `nil`, то код крашнется.

### Nil coalescing 

При проверке `optional` мы можем выставить дефолтное значение, если встретился `nil`.  

```swift
func connect(_ user: Int) -> String? {
    if user == 1 {
        return "Success"
    } else {
        return nil
...
let user = connect(12) ?? "Failed"
```

В данном случае, если функция возвращает `nil`, то с помощью `??` мы присвоим константе значение по умолчанию. 

### Optional chaining

Swift предоставляет возможность обращаться к методам, которые могут быть `optional` используя `.?`.   

К примеру, у массива есть метод `.first`, но метод является `optional`, так как у массива может не быть первого элемента.  

```swift
let users = ["KeoFoxy", "Alice", "PeaceDuke"]

let user = users.first?.uppercased()
```

Если метод не возвращает `nil`, то `uppercased()` отрабатывает.   

### Optional try  

`try` также можно использовать с optional.  
...
func checkPassword(_ password: String) throws -> Bool {
    if password == "123" {
        throw PasswordError.badPass
    }
    
    return true
}

if let result = try? checkPassword("123") {
print("Password is good")
} else {
    print("Password is bad. Try another one")
}
```

Если функция прокинет ошибку, то на выходе мы получим `nil` и перейдем в блок `else`. 

Также можно использовать ***force unwrapping***, если вы уверены, что функция вернет не `nil`.   

### Failable initializers  

**Failable initializer** - это инициализатор, который может не инициализировать объект, если переданные параметры не соответствуют ожидаемым условиям.

**Failable initializer** определяется с помощью ключевого слова `init?` вместо обычного `init`. Если инициализация объекта не удается, инициализатор вернет `nil`.

Пример использования **failable initializer** для инициализации объекта с опциональным значением:
...
let person1 = Person(name: "John") // инициализация объекта
let person2 = Person(name: "") // не удалось инициализировать объект, возвращает nil
```

**Failable initializer** может использоваться в тех случаях, когда необходимо убедиться, что переданные параметры соответствуют определенным условиям, или когда инициализация объекта может завершиться неудачно.

Важно помнить, что использование **failable initializer** может привести к появлению опциональных значений, поэтому необходимо проверять значения на `nil` перед использованием.

### Typecasting

**Typecasting** - это процесс приведения типа значения в Swift из одного типа в другой. Это может быть полезно, когда вы хотите использовать значение, которое было сохранено в одном типе, в контексте, где ожидается другой тип.

Swift предоставляет три способа для выполнения операции typecasting:

`as` operator - используется для приведения значения к конкретному типу. Это может быть достигнуто с помощью явного приведения типа, например, следующим образом:   
...
`as?` operator - используется для приведения типа значения до опционального типа. Если приведение типа не может быть выполнено, результат будет `nil`. Например:   

```swift
let myString = "Hello, World!"
let myInt = Int(myString) // nil
let myIntOptional = Int(myString) // Optional(nil)
```

`as!` operator - используется для приведения типа значения к необходимому типу. Если приведение типа невозможно, программа завершится с ошибкой выполнения. Например:   

```swift
let myString = "42"
let myInt = Int(myString)! // 42
```

Важно понимать, что typecasting может быть потенциально опасным, поскольку неправильное использование может привести к ошибкам выполнения. Поэтому, перед использованием операторов typecasting, следует тщательно проверять типы значений и убедиться в их правильности.